...
namespace CustomCompoundAmmoBoxes
{
    ...
    public class CustomCompoundAmmoBoxesScript : BaseUnityPlugin
    {
        ...
        private TextMeshPro current_ammo_box_tape_strip_text;
        ...
        {
            Debug.Log(string.Format("player interacted with {0}", gameobject.name));
                        ...
                        foreach (string asset_name in assetBundle.GetAllAssetNames())
                        {
                            ...
                            if (gameObject != null)
                            {
                                ...
                                if (ammo_box_script != null)
                                {
                                    ...
                                    var tape_strips_fallback = new GameObject("Tape Fallback");
                                    tape_strips_fallback.transform.parent = fallback_ammo_box.transform;
                                    tape_strips_fallback.transform.localPosition = new Vector3(0.125f, 0.012f, 0f);
                                    foreach (GameObject tape_strip in tape_strips)
                                    {
                                        tape_strip.gameObject.SetActive(false);
                                        current_tape_strips.Add(UnityEngine.Object.Instantiate(tape_strip, tape_strips_fallback.transform));
                                    }
                                    UnityEngine.Object.Instantiate(tape_strips_text, tape_strips_fallback.transform);
                                }
        ...
        private bool TryGetCustomAmmoBoxForCurrentRound()
        {
            ...
            foreach (GameObject ammo_box in custom_ammo_boxes)
            {
                if (ammo_box.GetComponent<ShootingRangeAmmoBoxScript>().round_prefab.GetComponent<ShellCasingScript>().InternalName == RCS.RoundPrefab.GetComponent<ShellCasingScript>().InternalName)
                {
                    ...
                    Debug.Log("Found an custom ammo box matching the current cartridge");
            ...
            if (!found_custom_ammo_box) //has a custom ammo box for the currect cartridge been found?
            {
                if (!RCS.RoundPrefab.GetComponent<ShellCasingScript>().InternalName.StartsWith("wolfire."))
                {
                    ...
                    current_ammo_box_tape_strip_text = current_ammo_box.GetComponentInChildren<TextMeshPro>(true);
                    current_ammo_box_tape_strip_text.text = RCS.RoundPrefab.name;
                    current_ammo_box_tape_strip_text.GetTextInfo(current_ammo_box_tape_strip_text.text); //even though it does it on its own when you instantiate an object I think you have to do it manually, so dumb.
                    if (current_tape_strips.Count != 0)
                    {
                        int i;
                        for (i = 0; i < current_tape_strips.Count - 1; i++)
                        {
                            var tape_strip_text_x = current_ammo_box_tape_strip_text.textBounds.extents.x;
